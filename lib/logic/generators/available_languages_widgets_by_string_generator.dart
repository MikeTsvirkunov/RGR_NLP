...
class AvailableLanguagesWidgetsByStringGenerator extends IStrategy{
  ...
  T execute<T>(IExtractable args) {
    List<String> availableLanguages = ContainerExtractor.extract(args, 'available_languages');
    return List.generate(availableLanguages.length, (index) => DropdownMenuItem(value: availableLanguages[index], child: LanguageItem(language: availableLanguages[index]))) as T;
  }
}